...
const Userlogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_SERVER_URL}/user/login`, { name: form.username, password: form.password }, { withCredentials: true });
      if (res.status === 200) {
      ...
      >
        <div className="flex justify-center items-center gap-8 w-full">
          <div className="flex flex-col items-center gap-3">
            <div className="text-4xl font-semibold text-[#262626]">User Login</div>
        ...
        <div className="flex items-center gap-2 my-5">
          ...
          <Link className="flex gap-2 items-center" to={'/userregister'}><div className="underline">Register</div><img className="h-3 w-auto" src="arrow-3.png" alt="arrow" /></Link>
        </div>
      </motion.div>
    </>
  )
