...
const Adminregister = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        setLoading(true);
        try {
            await axios.post(`${import.meta.env.VITE_SERVER_URL}/admin/register`, { name: form.adminname, password: form.password }, { withCredentials: true })
            console.log(form);
            navigate(`/adminlogin`);
            ...
            >
                <div className="flex justify-center items-center gap-8 w-full">
                    <div className="flex flex-col items-center gap-3">
                        <div className="text-4xl font-semibold text-[#262626]">Rgister</div>
                        <div className="text-[#4C4C4D] max-md:text-center">Create your new admin account</div>
                    </div>
                ...
                <div className="flex items-center gap-2 my-5">
                    ...
                    <Link className="flex gap-2 items-center" to={'/Adminlogin'}><div className="underline">Login</div><img className="h-3 w-auto" src="arrow-3.png" alt="arrow" /></Link>
                </div>
            </motion.div>
        </>
    )
